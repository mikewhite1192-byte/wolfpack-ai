...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Build email threads from CRM database first (always works)
    ...
    if (emailMessages.length > 0) {
      ...
      emailThreads = Object.entries(grouped).map(([subject, msgs]) => ({
        id: `crm-${msgs[0].id}`,
        subject,
        from: (msgs[0].sender as string) || "",
        to: (msgs[0].recipient as string) || "",
        date: msgs[msgs.length - 1].created_at as string,
    ...
    // Also try Gmail API for additional threads (if connected)
    try {
      ...
      if (contactEmail) {
        ...
        if (token) {
          ...
          if (threadList.threads && threadList.threads.length > 0) {
            ...
            emailThreads = [...emailThreads, ...gmailThreads.filter(Boolean)];
          }
        }
      }
    } catch { /* Gmail not connected, CRM emails still show */ }
